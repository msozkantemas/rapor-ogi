...
JSON verisinden, `top3`, `top3-sub`, `logo` ve `social-media` klasörlerindeki görselleri kullanarak tek sayfalık PDF raporu üretir.
...
- `top3-sub/1.*`, `2.*`, `3.*`: ilk 3 kartta ana fotoğrafın üstüne yuvarlak olarak bindirilecek ek görseller
...
```bash
node generate-report.js --data data.json --top3 top3 --top3-sub top3-sub --logo logo --social social-media --output rapor.pdf
```
